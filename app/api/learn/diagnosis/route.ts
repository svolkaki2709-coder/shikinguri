...
export async function GET() {
  ...
  const [catRows, expenseRow, assetRows, payslipRows, mortgageRows] = await Promise.all([
    // 直近1年のカテゴリ別支出（世帯全体）
  ...
  for (const r of catRows) categoryTotals[r.category] = Number(r.total)

  const annualExpense = Number(expenseRow[0]?.total ?? 0)
  ...
  return NextResponse.json({
    ...
    savings: Number(assetRows[0]?.savings ?? 0),
    investment: Number(assetRows[0]?.investment ?? 0),
